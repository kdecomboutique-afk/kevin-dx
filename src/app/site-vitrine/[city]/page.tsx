...
export async function generateMetadata({
  ...
  const title = `Site Vitrine à ${city.name} — Création Pro dès 599€ | Kevin DX`;
  const description = `Création de site vitrine professionnel à ${city.name}. Design sur mesure, SEO local ${city.name} et ${city.department}, score Lighthouse 95+. Devis gratuit. Basé ${city.distance} de ${city.name}.`;

  return {
    ...createMetadata({
      title,
      description,
      path: `/site-vitrine/${city.slug}`,
    }),
    other: {
      "script:ld+json": JSON.stringify([
        {
          ...
          "@type": "Service",
          name: `Création de site vitrine à ${city.name}`,
          description,
          provider: {
            "@type": "LocalBusiness",
          ...
          offers: {
            ...
            price: "599",
            priceCurrency: "EUR",
            description: `Site vitrine professionnel à ${city.name}`,
          },
        },
        {
          "@context": "https://schema.org",
          "@type": "LocalBusiness",
          name: SITE_CONFIG.name,
          url: `${SITE_CONFIG.url}/site-vitrine/${city.slug}`,
          telephone: CONTACT.phone,
          email: CONTACT.email,
          address: {
            "@type": "PostalAddress",
...
export default async function SiteVitrineCityPage({
  params,
}: {
  params: Promise<{ city: string }>;
}) {
