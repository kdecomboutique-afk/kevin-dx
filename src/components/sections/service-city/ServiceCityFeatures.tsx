...
import type { ServiceCityConfig, ServiceFeature } from "@/data/service-city-seo-pages";
import type { LocalCity } from "@/data/local-seo";

interface ServiceCityFeaturesProps {
  service: ServiceCityConfig;
  city: LocalCity;
}

function FeatureIcon({ icon }: { icon: ServiceFeature["icon"] }) {
  const paths: Record<ServiceFeature["icon"], string> = {
    ...
    target: "M12 8.25v-1.5m0 1.5c-1.355 0-2.697.056-4.024.166C6.845 8.51 6 9.473 6 10.608v2.513m6-4.871c1.355 0 2.697.056 4.024.166C17.155 8.51 18 9.473 18 10.608v2.513M15 9.75l-3-3m0 0-3 3m3-3v11.25",
...
export default function ServiceCityFeatures({ service, city }: ServiceCityFeaturesProps) {
  return (
    <section className="bg-surface py-20 lg:py-28">
      <Container>
        <SectionHeading
          badge="Fonctionnalités"
          title={`Ce que comprend votre ${service.name.toLowerCase()} à ${city.name}`}
          subtitle={`Chaque ${service.name.toLowerCase()} que je crée intègre les fonctionnalités essentielles pour performer sur le marché de ${city.name}.`}
        />
        ...
        <FadeIn delay={0.6}>
          <div className="mt-12 rounded-2xl border border-accent/20 bg-accent/5 p-6 lg:p-8">
            <h3 className="mb-4 text-lg font-semibold text-primary">Résultats concrets pour votre activité</h3>
